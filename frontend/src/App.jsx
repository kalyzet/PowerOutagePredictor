...
import { outageApi, excelApi, dashboardApi, predictionApi, trainingApi, evaluationApi } from './services/api';
import { 
  Zap, 
  UploadCloud, 
  Download, 
...
function App() {
  ...
  const handleTrain = async () => {
    if (!window.confirm("Apakah Anda yakin ingin melatih ulang model Machine Learning dengan data saat ini? Proses ini mungkin membutuhkan waktu.")) {
      return;
    }
    ...
    try {
      ...
      if (response.data.status === 'success') {
        ...
        addToast(response.data.message || "Gagal melatih model (mungkin data kurang).", "error");
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSubmit = {
        ...formData,
        jam_mati: formData.jam_mati.length === 5 ? `${formData.jam_mati}:00` : formData.jam_mati,
        jam_nyala: formData.jam_nyala.length === 5 ? `${formData.jam_nyala}:00` : formData.jam_nyala,
      };
        ...
        <div className="bg-slate-900 border border-slate-700 p-3 rounded-lg shadow-xl text-sm">
          ...
          {payload.map((entry, index) => (
            <p key={index} style={{ color: entry.color }}>
              {entry.name}: {entry.value} {entry.name.toLowerCase().includes('durasi') ? 'jam' : ''}
            </p>
          ))}
        </div>
      );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500 selection:text-white">
      ...
      <div className="fixed top-4 right-4 z-50 flex flex-col gap-2">
        {toasts.map(toast => (
          <div key={toast.id} className={`flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg border animate-in fade-in slide-in-from-right-4 ${
        ...
        <header className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">Manajemen Data & Analisis</p>
            </div>
          </div>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-6 shadow-lg max-w-2xl mx-auto">
              <div className="flex justify-between items-start mb-2">
                ...
                >
                  {trainLoading ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Settings className="w-3.5 h-3.5" />}
                  Latih Ulang Model
                </button>
              </div>
              <p className="text-slate-400 text-sm mb-6">
                Pilih tanggal untuk memprediksi probabilitas pemadaman dan estimasi durasi menggunakan model Machine Learning.
              </p>

              {trainResults && (
                <div className="mb-6 p-4 rounded-lg bg-indigo-900/20 border border-indigo-500/30 text-sm">
                  <h3 className="font-semibold text-indigo-300 mb-2">✅ Training Berhasil! (Menggunakan {trainResults.samples_used} sampel)</h3>
                  <div className="grid grid-cols-2 gap-4 text-slate-300">
                    <div>
                      <span className="text-xs text-slate-400">Model Klasifikasi:</span><br/>
                      {trainResults.results.best_classification_model}
                    </div>
                    <div>
                      <span className="text-xs text-slate-400">Model Regresi:</span><br/>
              ...
              <form onSubmit={handlePredict} className="flex gap-4 items-end mb-8">
                <div className="flex-1">
                  <label className="block text-xs font-medium text-slate-400 mb-1">Target Tanggal</label>
                  <input 
                    type="date" 
                    required
                ...
                >
                  {predictLoading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
                  Analisis
                </button>
                <button 
                  type="button" 
                ...
                >
                  {predictLoading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Calendar className="w-4 h-4" />}
                  Prediksi 7 Hari
                </button>
              </form>
              ...
              {prediction7DaysData && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                  <div className="p-5 rounded-xl border border-slate-700 bg-slate-900 mb-4 shadow-xl">
                    <h3 className="text-lg font-semibold mb-4 text-slate-200">Forecast 7 Hari Ke Depan</h3>
                    <div className="h-64 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={prediction7DaysData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                          ...
                          <Tooltip 
                            ...
                            formatter={(value) => [`${value}%`, 'Probabilitas Mati']}
                          ...
                          >
                            {
                              prediction7DaysData.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={entry.probabilitas > 50 ? '#ef4444' : '#10b981'} />
                              ))
                            }
                          </Bar>
                        </BarChart>
                ...
                <div className="animate-in fade-in zoom-in-95 duration-300">
                  <div className="p-5 rounded-xl border border-slate-700 bg-slate-800/50 mb-4">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-sm text-slate-400">Hasil Prediksi untuk:</p>
                        <p className="text-lg font-bold text-white">{predictionData.target_date}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-slate-400 mb-1">Keandalan Data</p>
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                          predictionData.reliability === 'Tinggi' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                          predictionData.reliability === 'Sedang' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' :
                          'bg-red-500/10 text-red-400 border-red-500/20'
                        }`}>
                          {predictionData.reliability === 'Tinggi' && '🟢'}
                          {predictionData.reliability === 'Sedang' && '🟡'}
                          {predictionData.reliability.includes('Rendah') && '🔴'}
                          <span className="ml-1">{predictionData.reliability}</span>
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-slate-900 rounded-lg border border-slate-800">
                        <p className="text-sm text-slate-400 mb-1">Probabilitas Mati</p>
                        <div className="flex items-end gap-2">
                          <p className={`text-3xl font-bold ${predictionData.prediksi_mati ? 'text-red-400' : 'text-green-400'}`}>
                            {predictionData.probabilitas}%
                          </p>
                          <p className="text-sm mb-1 text-slate-500">
                            ({predictionData.prediksi_mati ? 'Mati' : 'Aman'})
                          </p>
                        </div>
                      </div>
                      <div className="p-4 bg-slate-900 rounded-lg border border-slate-800">
                        <p className="text-sm text-slate-400 mb-1">Estimasi Durasi</p>
                        <p className="text-3xl font-bold text-white">
                          {predictionData.prediksi_durasi > 0 ? predictionData.prediksi_durasi + ' jam' : '-'}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-slate-500 text-center">
                    ...
                    <p>Catatan: Prediksi heuristik ini menggunakan pola historis sederhana (hari dalam seminggu, kejadian terbaru, dan rata-rata durasi).</p>
                  </div>
                </div>
              )}
            </div>
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-5 shadow-lg hover:border-slate-700 transition-colors">
                    <p className="text-sm font-medium text-slate-400 mb-1">Total Pemadaman</p>
                    <p className="text-3xl font-bold text-white">{stats.total_outages} <span className="text-sm font-normal text-slate-500">kali</span></p>
                  </div>
                  <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-5 shadow-lg hover:border-slate-700 transition-colors">
                    <p className="text-sm font-medium text-slate-400 mb-1">Total Durasi</p>
                    <p className="text-3xl font-bold text-white">{stats.total_duration} <span className="text-sm font-normal text-slate-500">jam</span></p>
                  </div>
                  <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-5 shadow-lg hover:border-slate-700 transition-colors">
                    <p className="text-sm font-medium text-slate-400 mb-1">Rata-rata Durasi</p>
                    <p className="text-3xl font-bold text-white">{stats.avg_duration} <span className="text-sm font-normal text-slate-500">jam/kejadian</span></p>
                  </div>
                  <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-5 shadow-lg hover:border-slate-700 transition-colors">
                    <p className="text-sm font-medium text-slate-400 mb-1">Pemadaman Terakhir</p>
                    <p className="text-2xl font-bold text-white mt-1">{stats.last_outage || '-'}</p>
                  </div>
                </div>
                  ...
                  <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-6 shadow-lg">
                    ...
                    <div className="h-72 w-full">
                      {charts.trend.length > 0 ? (
                        ...
                        <div className="h-full flex items-center justify-center text-slate-500">Belum ada data tren.</div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-6 shadow-lg">
                    ...
                    <div className="h-72 w-full">
                      {charts.time_distribution.length > 0 ? (
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={charts.time_distribution} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#334155" vertical={false} />
                            <XAxis dataKey="waktu" stroke="#94a3b8" fontSize={12} tickLine={false} axisLine={false} />
                        ...
                        <div className="h-full flex items-center justify-center text-slate-500">Belum ada data distribusi.</div>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-4 transition-all duration-500 animate-in fade-in slide-in-from-left-4">
                  <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-6 shadow-2xl">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-medium text-slate-400 mb-1">Jam Mati</label>
                          <input 
                            type="time" 
                            name="jam_mati"
                            required
                        ...
                        <div>
                          <label className="block text-xs font-medium text-slate-400 mb-1">Jam Nyala</label>
                          <input 
                            type="time" 
                            name="jam_nyala"
                            required
                  ...
                  <div className="flex-1 overflow-auto custom-scrollbar p-2">
                    {loading && outages.length === 0 ? (
                      ...
                      <div className="h-full flex flex-col items-center justify-center text-slate-500 space-y-4">
                        ...
                        <p>Belum ada catatan pemadaman. Tambahkan data atau impor dari Excel.</p>
                      </div>
                    ) : (
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="sticky top-0 bg-slate-900/95 backdrop-blur z-10 text-xs uppercase tracking-wider text-slate-400">
                            <th className="px-4 py-3 font-medium rounded-tl-lg">Tanggal</th>
                            <th className="px-4 py-3 font-medium">Jam Mati</th>
                            <th className="px-4 py-3 font-medium">Jam Nyala</th>
                            <th className="px-4 py-3 font-medium">Durasi (jam)</th>
                        ...
                        <tbody className="text-sm">
                          {outages.map((item) => (
                            ...
                            >
                              <td className="px-4 py-4 font-medium text-slate-300">{item.tanggal}</td>
                              <td className="px-4 py-4 text-slate-400">{item.jam_mati}</td>
                              <td className="px-4 py-4 text-slate-400">{item.jam_nyala}</td>
        ...
        {activeTab === 'eval' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-4">
              ...
              >
                {evalLoading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Settings className="w-4 h-4" />}
            ...
            {evalMetrics && evalMetrics.summary && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-6 shadow-lg">
                  <h3 className="text-lg font-semibold mb-4 text-slate-200">Performa Nyata (Real-World)</h3>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 bg-slate-800/50 rounded-lg">
                        <p className="text-xs text-slate-400">Total Dievaluasi</p>
                        <p className="text-2xl font-bold text-white">{evalMetrics.summary.total_evaluated}</p>
                      </div>
                      <div className="p-4 bg-slate-800/50 rounded-lg">
                        <p className="text-xs text-slate-400">Akurasi Prediksi</p>
                        <p className="text-2xl font-bold text-green-400">
                          {evalMetrics.summary.classification?.accuracy 
                            ? (evalMetrics.summary.classification.accuracy * 100).toFixed(1) + '%' 
                            : '-'}
                        </p>
                      </div>
                      <div className="p-4 bg-slate-800/50 rounded-lg">
                        <p className="text-xs text-slate-400">F1 Score (Klasifikasi)</p>
                        <p className="text-2xl font-bold text-cyan-400">
                          {evalMetrics.summary.classification?.f1_score 
                            ? evalMetrics.summary.classification.f1_score.toFixed(2) 
                            : '-'}
                        </p>
                      </div>
                      <div className="p-4 bg-slate-800/50 rounded-lg">
                        <p className="text-xs text-slate-400">MAE (Error Durasi)</p>
                        <p className="text-2xl font-bold text-indigo-400">
                          {evalMetrics.summary.regression?.mae 
                            ? evalMetrics.summary.regression.mae.toFixed(2) + ' jam' 
                            : '-'}
                        </p>
                      </div>
                    </div>
                ...
                <div className="bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-slate-800 p-6 shadow-lg">
                  <h3 className="text-lg font-semibold mb-4 text-slate-200">Tren Akurasi</h3>
                  <div className="h-48 w-full">
                    {evalMetrics.chart_data && evalMetrics.chart_data.length > 0 ? (
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={evalMetrics.chart_data} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#334155" vertical={false} />
                          <XAxis dataKey="tanggal" stroke="#94a3b8" fontSize={12} tickLine={false} axisLine={false} />
                          <YAxis stroke="#94a3b8" fontSize={12} tickLine={false} axisLine={false} domain={[0, 100]} />
                          <Tooltip content={<CustomTooltip />} cursor={{ stroke: '#475569', strokeWidth: 1, strokeDasharray: '4 4' }} />
                      ...
                      <div className="h-full flex items-center justify-center text-slate-500">Belum ada data evaluasi.</div>
                    )}
                  </div>
                </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-900/95 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-800">
                      ...
                      <th className="px-4 py-3 font-medium">Prediksi (Prob)</th>
                  ...
                  <tbody className="text-sm">
                    {evalLogs.length === 0 ? (
                      <tr>
                        <td colSpan="6" className="px-4 py-8 text-center text-slate-500">Belum ada histori prediksi. Lakukan prediksi di tab Prediksi.</td>
                      </tr>
                    ) : (
                      evalLogs.map((log) => (
                        <tr key={log.id} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                          <td className="px-4 py-3 text-slate-400">{log.tanggal_prediksi}</td>
                          <td className="px-4 py-3 font-medium text-slate-200">{log.tanggal_target}</td>
                          <td className="px-4 py-3 text-slate-400 text-xs">{log.model}</td>
                          <td className="px-4 py-3">
                            <span className={log.prediksi_mati ? 'text-red-400' : 'text-green-400'}>
                              {log.prediksi_mati ? 'Mati' : 'Aman'} ({log.probabilitas}%)
                            </span>
                          </td>
                          <td className="px-4 py-3">
                            {log.aktual_mati === null ? '-' : (
                              <span className={log.aktual_mati ? 'text-red-400' : 'text-green-400'}>
                          ...
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                              log.status === 'Selesai' && log.prediksi_benar 
                              ...
                              {log.status === 'Selesai' ? (log.prediksi_benar ? 'Benar ✅' : 'Salah ❌') : log.status}
                            </span>
                          </td>
                        </tr>
                      ))
